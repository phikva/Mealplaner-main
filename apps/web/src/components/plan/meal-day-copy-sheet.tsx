...
export function MealDayCopySheet({
  ...
  const runCopy = (dates: string[]) => {
    ...
    startTransition(async () => {
      const res = await copyMealPlanDayToDatesAction({
        sourcePlanDate,
        targetDates: dates,
      });
      if (!res.ok) {
      ...
      <Dialog.Portal>
        ...
        >
          ...
          <div className="shrink-0 space-y-1 border-b border-border/40 px-4 pb-3 pt-1 md:px-5 md:pt-3">
            <Dialog.Title className="text-base font-semibold tracking-tight">Kopier dag</Dialog.Title>
            <Dialog.Description className="text-sm text-muted-foreground">
              Kopierer {mealCount} måltid i samme rekkefølge (Måltid 1/2/3…). Eksisterende måltid i samme slot blir ikke
              overskrevet.
            </Dialog.Description>
          </div>

          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4 md:px-5">
            <p className="text-sm font-semibold text-foreground">{sourcePlanDate}</p>
            ...
            <div className="space-y-2 rounded-xl border border-border/50 bg-muted/10 p-3">
              ...
              <p className="text-xs text-muted-foreground">
                {restOfWeek.length > 0
                  ? `${restOfWeek.length} dag(er) etter ${sourcePlanDate.slice(8, 10)}.${sourcePlanDate.slice(5, 7)} i samme uke.`
            ...
            <div className="space-y-2 rounded-xl border border-border/50 bg-muted/10 p-3">
              <label htmlFor="copy-day-forward-days" className="text-xs font-semibold text-muted-foreground">
                Fremover i tid
              </label>
              <p className="text-xs text-muted-foreground">Antall påfølgende dager (etter denne dagen).</p>
